...
public class ConexionCliente extends Thread {
	...
	public void run() {
		...
		String vacio="";
		System.out.println("PULSA ENTER PARA RECIBIR EL TAMAÑO Y VALORES DE LOS ARRAYS");
		sc.nextLine();
	
		System.out.println("El servidor esta enviando datos para crear arrays del cliente");
		try {
			int filasArray=this.escucha.readInt();
			int columnasArray=this.escucha.readInt();
			// construimos nuestro array y lo rellenamos con la infor que envia servidor
			enteros= new int[filasArray][columnasArray];
			
			for (int i = 0; i < this.enteros.length; i++) {
				for (int j = 0; j < this.enteros[i].length; j++) {
					
					//escuchamos valor y lo seteamos en el array
					this.enteros[i][j]=this.escucha.readInt();
				}
				
			}
			// vamos a recibir filas y columnas para array 2
			
			filasArray= this.escucha.readInt();
			columnasArray=this.escucha.readInt();
			
			// construimos el array 
			decimal= new double[filasArray][columnasArray];
			
			for (int i = 0; i < decimal.length; i++) {
				for (int j = 0; j < decimal[i].length; j++) {
					//escuchamos valores y seteamos en array
					this.decimal[i][j]= this.escucha.readDouble();
			...
			// si eleccion es 2 queremos setear informacion
				...
				if (matriz == 1) {
					...
					try {		
						...
						this.enteros[fila-1][columna-1]=numero;
						if (seteo == 100) {
				...
				// trabajaremos la matriz2
					...
					try {
						...
						numero=Math.round(numero*100.0)/100.0;
							...
							this.decimal[fila-1][columna-1]=numero;
