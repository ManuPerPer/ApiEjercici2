...
public class SirveACliente extends Thread {
	...
	private int[][] enteros = new int[4][4];
	private double[][] decimal = new double[3][3];
	...
	public void run() {
		...
		try {
			...
			//enviamos tamaño de filas para matriz1
			this.escribe.writeInt(this.enteros.length);
			//enviamos tamaño de columnas para matriz1
			this.escribe.writeInt(this.enteros[1].length);
			//enviamos valores para rellenar array1
			for (int i = 0; i < this.enteros.length; i++) {
				for (int j = 0; j < this.enteros[i].length; j++) {
					this.escribe.writeInt(this.enteros[i][j]);
				}
				
			}
			
			//enviamos tamaño de filas para matriz2
			this.escribe.writeInt(this.decimal.length);
			//enviamos tamaño de columnas para matriz2
			this.escribe.writeInt(this.decimal[1].length);
			
			//enviamos valores para rellenar array2
			for (int i = 0; i < this.decimal.length; i++) {
				for (int j = 0; j < this.decimal[1].length; j++) {
					this.escribe.writeDouble(this.decimal[i][j]);
	...
	public int esperaColumnaMatriz2() throws IOException {
		...
		do {
			...
			if (columna > 0 & columna <= this.decimal[1].length) {
				this.escribe.writeInt(this.ok);
			} else {
				this.escribe.writeInt(this.fallo);
			}
		} while (columna <= 0 || columna >this.decimal[1].length);
		return columna;
	}
	...
	public int esperaFilaMatriz2() throws IOException {
		...
		do {
			...
			if (fila > 0 && fila <= this.decimal.length) {
				this.escribe.writeInt(this.ok);
			} else {
				this.escribe.writeInt(this.fallo);
			}
		} while (fila <= 0 || fila >this.decimal.length);
		return fila;
	}
	...
	public int esperaColumnaMatriz1() throws IOException {
		...
		do {
			...
			if (columna >= 1 && columna <= this.enteros[1].length) {
				this.escribe.writeInt(this.ok);
			} else {
				this.escribe.writeInt(this.fallo);
			}
		} while (columna <= 0 || columna >this.enteros[1].length);
		return columna;
	}
	...
	public int esperaFilaMatriz1() throws IOException {
		...
		do {
			...
			if (fila >= 1 && fila <=this.enteros.length) {
				this.escribe.writeInt(this.ok);
			} else {
				this.escribe.writeInt(this.fallo);
			}
		} while (fila <= 0 || fila > this.enteros.length);
		return fila;
	}
